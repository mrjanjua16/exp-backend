...
export default class CategoriesController {
    async create({ request, auth, response }: HttpContext) {
        try {
            ...
            const category = await CategoryService.add(valid_category, auth.user?.id || Number.MIN_SAFE_INTEGER);
    ...
    async update({ request, params, auth, response }:HttpContext) {
        try {
            ...
            const updated = await CategoryService.update(params.id, valid_update, auth.user?.id || Number.MIN_SAFE_INTEGER);
    ...
    async delete({ params, auth, response }: HttpContext) {
        const deleted = await CategoryService.delete(params.id, auth.user?.id || Number.MIN_SAFE_INTEGER);

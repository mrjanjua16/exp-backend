...
import { categoryValidator, createPlanned, monthValidator } from '#validators/ledger_management/ledger'
...
export default class IncomesController {
    async addPlanned({ request, auth, response }: HttpContext) {
        try {
            ...
            const plannedAmount = await LedgerService.createLedger(month, category_id, planned, actual, user_id, user_id);

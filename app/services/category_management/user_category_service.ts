...
export class UserCategoriesService {
    ...
    static async create(category_ids: number[], user_id: number) {
        ...
        for (const category_id of category_ids) {
            ...
            if (valid_category.deleted_by !== null) {
                errorMessages.push(`Category with ID ${category_id} is inactive, please contact administrator!`);
                continue; // Skip this category and move to the next
            }
    ...
    static async remove(category_ids: number[], user_id: number) {
        ...
        for (const category_id of category_ids) {
            ...
            if (valid_category.deleted_by !== null) {
                return { error: { status: 404, message: 'Category is inactive, please contact administrator!' } };
            }
        }
    ...
    static async list(user_id: number) {
        ...
        const user_categories = await valid_user.related('categories').query().whereNull('categories.deleted_by').exec();

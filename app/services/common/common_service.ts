...
export class CommonServices {
    ...
    static async validUser(user_id: number) {
        ...
        if (!valid_user) { throw { error: { code: 404, message: 'User not found!'}}};
    ...
    static async validMonth(month: string) {
        ...
        if (!valid_month) { throw { error: { code: 400, message: 'Provide correct date.'}}};
        return month;
    }
    ...
    static async validCategory(category_id: number) {
        ...
        if (!valid_category) { throw { error: { code: 404, message: 'Category not found.'}}};

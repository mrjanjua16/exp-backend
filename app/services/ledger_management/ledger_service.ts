...
export class LedgerService {
     ...
     */
    static async createLedger(month: string, category_id: number, planned: number, actual: number, user_id: number, auth_user: number) {
        ...
        const valid_plan = await Income.query().where('user_id', valid_user).andWhere('category_id', valid_category).andWhere('month', valid_month).first();
        if (!valid_plan) {
            ...
            const income = await Income.query().increment({
                planned: +planned, actual: +actual
            }).where('user_id', valid_user).andWhere('category_id', valid_category).andWhere('month', valid_month);
            return income;
        }
    }
    ...
    static async listLedgerPerMonth(month: string, user_id: number) {
        ...
        if (!ledger) {return {error: {code: 404, message: 'No income ledger found.'}}};
        const ledger_details = await Promise.all(ledger.map(async (ledger) => {
    ...
    static async listLedgerPerCategory(category_id: number, user_id: number) {
        const valid_category = await CommonServices.validCategory(category_id);
        const ledger = await Income.query().where('user_id', user_id).andWhere('category_id', valid_category);
        if (!ledger) {return {error: {code: 404, message: 'No income ledger found.'}}};
        return ledger;
    }
}